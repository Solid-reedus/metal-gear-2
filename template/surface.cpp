...
bool Tmpl8::Surface::RayCastHit(int2 p_start, int2 p_end) const
{
	...
	while (true)
	{
		// Bounds check: ensure x and y are within valid pixel range
		if (x < 0 || x >= width || y < 0 || y >= height)
		{
			return false; // Out of bounds, consider this a hit (or missed ray)
		}

		// Access the pixel at (x, y)
		if (pixels[y * width + x] != TRANSPARENCY_MASK)
		{
			return false; // Hit detected
		}

		// If we've reached the end point, return true (no collision)
...
void Surface::CopyTo( Surface* d, int x, int y )
{
	...
	if ((src) && (dst))
	{
		...
		if ((srcwidth > 0) && (srcheight > 0))
		{
			...
			{
				memcpy( dst, src, srcwidth * sizeof(int));
